...
PUBLIC int fs_readwrite(void)
{
  ...
  struct inode *rip;
  size_t nrbytes;
  uid_t caller = caller_uid = (uid_t) fs_m_in.REQ_UID;
  ...
  while (nrbytes > 0) {
	  ...
	  r = rw_chunk(rip, cvul64((unsigned long) position), off, chunk,
	  	       nrbytes, rw_flag, gid, cum_io, block_size, &completed, caller);
...
PUBLIC int fs_breadwrite(void)
{
  ...
  while (nrbytes > 0) {
	  ...
	  r = rw_chunk(&rip, position, off, chunk, nrbytes, rw_flag, gid,
	  	       cum_io, block_size, &completed, caller);
...
PRIVATE int rw_chunk(rip, position, off, chunk, left, rw_flag, gid,
 buf_off, block_size, completed, caller)
...
/* Read or write (part of) a block. */
  ...
  int encrypting = 0; /* Keeps track of whether we do encryption */
  u16_t i_mode = rip->i_mode; /* Holds the i_mode of our file */
  u16_t i_uid = rip->i_uid; /* Hold the user id of who owns the file */
  unsigned long rk[RKLENGTH(KEYBITS)];	/* round key */
  unsigned char key[KEYLENGTH(KEYBITS)];/* cipher key */
  int ctr, k0, k1, i;
  int fileId = (int) rip->i_num;   /* Grab the inode number to use for the fileID */
